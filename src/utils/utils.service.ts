...
export class UtilsService {
  ...
  static replaceElements(data, key, toReplace) {
    ...
    if (!_.isNil(newObject[key])) {
      newObject[key] = toReplace.find((element) => element.id === data[key].id);
    }
    return newObject;
  }
  ...
  static replaceWith(data, key, toReplace) {
    if (Array.isArray(data)) {
      return data.map((obj) => {
        ...
        newInternalObj[key] = toReplace.find(
          (element) => element.id == obj[key]
        );
  ...
  static filterSimpleId(data, collectionName, idKey): Array<any> {
    return [
      ...new Set(
        _.map(data[collectionName], idKey).filter(
  ...
  filterInternalIds(collection, containerKey, idKey): Array<number> {
    ...
    const objectReduce = (accumulator, obj) =>
      accumulator.add(_.get(obj, idKey));
    const internalReduce = (accumulator, internal) =>
      _.reduce(internal[containerKey], objectReduce, accumulator);
  ...
  static translateOrderQuery(
    ...
    if (query.order) {
      ...
      let translated;
      if (findIn === "value") {
        translated = _.keys(currentDictionary).find(
          (key) => currentDictionary[key] === field
        );
  ...
  static mergeDynamicAttributes(
    data: { extra_attributes: { [x: string]: any } },
    ...
    if (extraAttributes) {
      ...
      const completedExtraAttributes = slugs.map((slug) => {
        const { data_type: dataType, name } = dynamicAttributes.find(
          (dynamicAttribute) => dynamicAttribute.slug === slug
        );
